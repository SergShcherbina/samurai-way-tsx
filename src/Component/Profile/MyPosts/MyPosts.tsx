...
export const MyPosts: React.FC<MyPostType> = ({ addPost, posts }) => {
  ...
  const postElement = posts.map((el) => (
    <Post id={el.id} key={el.id} message={el.message} counterLike={el.likeCounter} counterDislike={el.counterDislike} />
  ));
  ...
  return (
    <div>
      ...
      <div ref={refDivAnimate}>{postElement}</div>
    </div>
  );
};
...
const AddPostForm: React.FC<InjectedFormProps<AddPostFormDataType>> = (props) => {
  return (
    <form onSubmit={props.handleSubmit}>
      <div>
        <Field component={Textarea} name="addPostProfile" validate={[requiredField]} placeholder="Введите текст" />
      </div>
