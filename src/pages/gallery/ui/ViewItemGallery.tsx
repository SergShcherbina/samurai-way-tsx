...
import {galleryActions} from "../model/gallery-actions";
...
export const ViewItemGallery: FC<PropsType> = ({activeId, open, onClose}) => {
    ...
    const likeHandler = (id: string) => {
        dispatch(galleryActions.likedAC(Number(id)));
    };

    const dislikeHandler = (id: string) => {
        dispatch(galleryActions.dislikedAC(Number(id)));
    };

    useEffect(() => {
        console.log('useEffect')
        dispatch(galleryActions.setViewsAC(Number(activeId)));
    }, [activeId]);
